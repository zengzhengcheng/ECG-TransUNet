...
def test_model(model, device,test_loader,epoch,step,img_dir,logger, accurate_data_dir=None, batch_size=16):
    model.eval()  # 设置为评估模式
    ...
    with torch.no_grad():  # 测试时不计算梯度
        ...
        for teststep, (data, labels, paths, start_indices) in enumerate(test_loader):
            ...
            for j in range(num_windows):
                ...
                pred_dict = {
                    # 'clean_ecg': outputs['clean_ecg'],
                    'pred_r': pred_r_peak,
                }
                # 包装 Targets
                target_dict = {
                    # 'clean_ecg': current_recon_target,  # 无论是合成还是原始
                ...
                with torch.no_grad():
                    ...
                    target, weight_map = generate_weight_from_labels(normalized_labels, is_soft_label=True, to_tensor=True)
                ...
                metrics = logger.log_step(
                    mode='test',
                    epoch=epoch,  # 1. Epoch
                    global_step=teststep,  # 2. Global Step
                    step=j,  # 3. Step in loop
    ...
    visualize_and_save(pwindow_data.cpu().numpy(), pwindow_labels.cpu().numpy(),
                       ppred_labels, epoch,step, img_dir,accuracy)
    # gan_visualize_and_save(pwindow_data.cpu().numpy(), pwindow_labels.cpu().numpy(),
    #                        ppred_labels, ppred_window_data.cpu().numpy(), epoch, step, img_dir)
...
def train_model(model, device,train_loader,  criterion, num_epochs=10,
                ...
                update_every=5, test_every=20, img_dir='img',model_dir='models',gan=False,prefix="", accurate_data_dir=None):
    ...
    for epoch in range(start_epoch,start_epoch+num_epochs):
        ...
        for i, (data, labels, paths, start_indices) in tqdm(enumerate(train_loader), total=len(train_loader), desc=f"Epoch {epoch+1}/{num_epochs}"):
            ...
            if (i + 1) % test_every == 0:
                ...
                test_model(model,device,test_loader,epoch,step,img_dir,logger, accurate_data_dir, batch_size=batch_size)  # 调用test函数进行测试
...
if __name__ == '__main__':
    ...
    testdataset = ECGDataset(data_dir=data_dir, expanded=False, window_size=307200, stride=153600, test_size=0.1,
                             wave=True)
    traindataset = ECGDataset(data_dir=data_dir, expanded=False, window_size=307200, stride=153600, test_size=0.1,
                              wave=True)
