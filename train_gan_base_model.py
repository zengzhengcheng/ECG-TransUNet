...
from ablistimunet.losses import AdvancedGenerativeLoss, AdvancedSegmentationLoss, AggressiveSegmentationLoss
...
def test_model(model, ganmodel,device,test_loader,epoch,step,img_dir,logger, accurate_data_dir=None, batch_size=16):
    model.eval()  # 设置为评估模式
    ...
    if accurate_data_dir:
        ...
        evaluator.evaluate(model, device, accurate_data_dir, batch_size=batch_size, gan_model=ganmodel)
    
    return all_preds_dict,labels
# 训练函数
def train_model(model, ganmodel,device,train_loader, criterion, num_epochs=10,
    ...
    for epoch in range(start_epoch,start_epoch+num_epochs):
        ...
        for i, (data, labels, paths, start_indices) in tqdm(enumerate(train_loader), total=len(train_loader), desc=f"Epoch {epoch+1}/{num_epochs}"):
            ...
            if (i + 1) % test_every == 0:
                ...
                test_model(model,ganmodel,device,test_loader,epoch,step,img_dir,logger, accurate_data_dir, batch_size=batch_size)  # 调用test函数进行测试
...
if __name__ == '__main__':
    ...
    agument = ECGDataAugmentation(augment=True, expanded=True)
    ganagument = ECGDataAugmentation(augment=False,expanded=False)
    model.to(device)
    ...
    train_model(model,ganmodel,device, train_loader, criterion, num_epochs=10,start_epoch=0,
    ...
    train_loader = ECGDataLoader(traindataset, batch_size=8, shuffle=True, test=False,num_workers=4)
    test_loader = ECGDataLoader(testdataset, batch_size=1, shuffle=True, test=True)
    print(f"Number of training samples: {len(train_loader)}", f"Number of test samples: {len(test_loader)}")
    train_model(model, ganmodel, device, train_loader, criterion, num_epochs=10, start_epoch=10,
