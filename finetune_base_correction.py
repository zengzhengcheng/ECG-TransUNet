...
dir_list = ["common", "dirbase", "dirbase1", "dirbase2"]

dir_name = dir_list[1]
machine = machine_list[1]
# 获取配置信息
data_dir = config.get(machine, 'data_dir')
...
window_stride = config.getint(dir_name, 'window_stride')
...
# 测试函数 - 按照数据分数分别记录
def test_model(model, device, test_loader, logger, epoch, step, img_dir, batch_size=8):
    model.eval()  # 设置为评估模式
    ...
    with torch.no_grad():  # 测试时不计算梯度
        ...
        for teststep, (data, labels, paths, start_indices, scores) in enumerate(test_loader):
            ...
            for j in range(num_windows):
                ...
                accuracy_dict = calc_ecg_accuracy_detailed(pred_r_peak.reshape((-1, 15360)), window_labels)
                ...
                with torch.no_grad():
                    ...
                    # BCE per pixel: (B, 1, L)
                    normalized_labels = normalize_input_shape(window_labels)
                    raw_bce = mining_criterion_bce(pred_r_peak, normalized_labels)
                    # 加上 weight map (关注困难区域) - 与训练过程保持一致
                    target, weight_map = generate_weight_from_labels(normalized_labels, is_soft_label=True,
                                                                     to_tensor=True)
        ...
        for score, metrics_list in score_metrics.items():
            if metrics_list:
                ...
                print(f"分数 {score}: 准确率={avg_precision:.4f}, 召回率={avg_recall:.4f}, F1={avg_f1:.4f}")
    ...
    if (pwindow_data is None):
        ...
        paccuracy = accuracy
        ppred_labels = pred_labels
    visualize_and_save(pwindow_data.cpu().numpy(), pwindow_labels.cpu().numpy(),
                       ppred_labels, epoch, step, img_dir, paccuracy)

    return all_preds_dict, labels


# 训练函数
def train_model(model, device, train_loader, test_loader, criterion, logger, num_epochs=10,
    ...
    for epoch in range(start_epoch, start_epoch + num_epochs):
        model.train()
        epoch_loss = 0
        for i, (data, labels, paths, start_indices, scores) in tqdm(enumerate(train_loader), total=len(train_loader),
            ...
            for j in range(num_windows):
                ...
                wavelet_feats_np = ECGFeatureExtractor.extract_wavelet_features(window_data)
                wavelet_tensor = torch.from_numpy(wavelet_feats_np).float().to(device)  # (2, L)
                target, weight_map = generate_weight_from_labels(window_labels, is_soft_label=False, to_tensor=True)
                weight_map = weight_map.to(device)
                window_data = window_data.to(device)
                # 前向传播
                optimizer.zero_grad()
                ...
                if (j + 1) % update_every == 0 or (j + 1) == num_windows:
                    accuracy_dict = calc_ecg_accuracy_detailed(pred_r_peak.squeeze(), window_labels)
                    accuracy = accuracy_dict['acc_precision']
            ...
            if (i + 1) % test_every == 0:
                ...
                test_model_func(model, device, test_loader, logger, epoch, step, img_dir,
                                batch_size=batch_size)  # 调用test函数进行测试
...
def fine_tune_model(model_type, model_path, train_loader, test_loader):
    ...
    logger = TrainingLogger(
        log_dir=model_dir,
        train_name=f"correct_{model_type}",
        heavy_mode=True
    )
    ...
    # 开始微调
    train_model(model, device, train_loader, test_loader, criterion, logger, num_epochs=10, start_epoch=0,
                update_every=50, test_every=20, img_dir=img_dir, model_dir=model_dir,
                prefix=f'correct_{model_type}', test_model_func=test_model)
...
if __name__ == '__main__':
    ...
    # 定义base模型路径
    base_model_path = os.path.join("./modelAll/modeldirbase2/", 'model_7_1448.pth')

    # 微调base模型
    if os.path.exists(base_model_path):
        fine_tune_model('base', base_model_path, train_loader, test_loader)
    else:
        print(f"模型文件不存在: {base_model_path}")