    ...
    for epoch in range(start_epoch, start_epoch + num_epochs):
        ...
        for i, (data, labels, paths, start_indices, scores) in tqdm(enumerate(train_loader), total=len(train_loader), desc=f"Epoch {epoch+1}/{num_epochs}"):
            ...
            if (i + 1) % test_every == 0:
                ...
                test_model_func(model, device, test_loader, logger, epoch, step, img_dir, batch_size=batch_size)  # 调用test函数进行测试
...
def fine_tune_model(model_type, model_path, train_loader, test_loader):
    ...
    # 开始微调
    train_model(model, device, train_loader, test_loader, criterion, logger, num_epochs=20, start_epoch=0,
...
if __name__ == '__main__':
    ...
    # 定义最新模型路径
    latest_model_path = os.path.join("./modelAll/modelgan1/", 'model_9_1810.pth')
    
    # 微调最新模型
    if os.path.exists(latest_model_path):
        fine_tune_model('gan', latest_model_path, train_loader, test_loader)
    else:
